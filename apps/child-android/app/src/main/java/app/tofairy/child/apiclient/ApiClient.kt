...
/**
 * 백엔드 계약 클라이언트 (CLAUDE.md §10, `docs/40` 계약 소비).
 *
 * 자녀 앱이 의존하는 요청/relay 계약만 노출한다. 부모 전용 인증과 기기 승인 API는 부모 앱
 * 책임이다. digest mailbox/history 조회 API는 존재하지 않는다.
 *
 * 불변식:
 *  - #1: 자녀 콘텐츠/행동 산출물 송신은 digest 모듈을 통해서만 가능하다. apiclient는
 *        [EncryptedDigest]만 활성 relay로 전달하며 평문 다이제스트를 받지 않는다.
 *        consent/pairing/device/routing 같은 허용된 운영 metadata 계약은 별도로 존재한다.
 *  - #6: [consentStatus] 는 센싱 게이트의 권위 있는 소스(`docs/40` §3 `GET /v1/consent/status`).
 * 백엔드 미구현 동안 [MockApiClient] 로 앱을 선개발한다.
 */
interface ApiClient {

    /**
     * `docs/40` §3 — `GET /v1/consent/status`.
    ...
    /**
     * `docs/40` §4.1 — `POST /v1/pairing/claim`.
    ...
    /**
     * 인증된 parent relay 연결에서 온 ACK만 전달하는 구독 계약이다. transport 구현은 연결의
     * parent device identity와 현재 trusted set을 검증한 뒤 listener를 호출해야 한다.
     */
    fun subscribeDigestAcks(listener: DigestAckListener): DigestAckSubscription
}

/** transport가 인증한 부모 기기의 논리 `DIGEST_ACK`. */
data class DigestAck(
    val digestId: String,
...
fun interface DigestAckListener {
    suspend fun onDigestAck(ack: DigestAck)
}

fun interface DigestAckSubscription {
    fun cancel()
}
